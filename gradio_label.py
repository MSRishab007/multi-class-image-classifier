...
# Only keep the attributes you want
attributes = ["color", "material", "condition"]

# --- Config ---
input_folder = r"C:\Users\Nimish\Downloads\DL_project\images"   # your images folder
...
index = 0  # global pointer
rows = []  # store results in memory before saving

# Track splits
split_counts = defaultdict(lambda: {"train": 0, "val": 0})
instance_split = {}  # maps instance_id -> split

# --- Helper: parse filename ---
def parse_filename(fname):
    """
    Example: team9_cap_1_a.jpg
    -> class_label = cap
    -> instance_id = cap_1
    """
...
# --- Functions ---
def get_image(idx):
    """Return image and filename at index"""
    if idx < len(files):
        filepath = os.path.join(input_folder, files[idx])
        return filepath, files[idx]
    return None, "Done"

def submit_label(color, material, condition, caption, split):
...
with gr.Blocks() as demo:
    gr.Markdown("## 🏷️ Image Labeling Tool with Progress and Split Enforcement")
    ...
    submit_btn.click(
        fn=submit_label,
        inputs=[color_dd, material_dd, condition_dd, caption_box, split_dd],
